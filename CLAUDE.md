...
5. **Notes backends** (`src/notes/`) — `NotesBackend` implementations over WebDAV or the local filesystem, selected via `NOTES_BACKEND`. Reads span `NOTES_ROOT`; writes are confined to `MEMORY_ROOT` **and** `SKILLS_ROOT` by a code-enforced prefix check (`assertWritable` takes the list of writable prefixes, not a single one). Both default under `<NOTES_ROOT>/betty/`, so memory is always a strict subset of the user's notes and everything Betty owns sits in one folder. Every write carries a real `If-Match` ETag and fails loudly on conflict rather than clobbering a concurrent human edit. Memory files use Google's Open Knowledge Format (OKF v0.1) — markdown with YAML frontmatter, one concept per file. A `SKILL.md` under the skills root is the one exception: it gets `name`/`description` frontmatter instead, because `list_skills` skips a folder whose manifest has neither.
...
- **Read-wide, write-narrow**: notes reads may span `NOTES_ROOT`; writes are prefix-checked against `MEMORY_ROOT` and `SKILLS_ROOT` and rejected otherwise. Enforced in code, never merely described in a tool description. There is deliberately **no whole-file write or overwrite tool** — the absence is the safety mechanism, so don't add one. Widening the write scope further needs a deliberate decision: the point is that the user's own notes stay readable and untouchable.
- **Skills are instructions, not code**: Betty reads markdown from `SKILLS_ROOT`, and may now write it there too — but nothing in a skill's `scripts/` directory is ever read or executed. Writability is about authoring instructions, and does not loosen that.
