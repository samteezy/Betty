...
`betty-mcp` is an MCP server for **Betty** — an assistant layer that travels between agentic platforms. Betty's memory and skills live in the user's own file storage rather than inside any one platform, so they move with them. Documentation is three files, and each is authoritative for its own surface: **[README.md](README.md)** for supported backends, configuration, setup, and remote access; **[docs/memory.md](docs/memory.md)** for the memory system — roots, recall, OKF, `index.md`, the desk, and the bundled skills; **[docs/tools.md](docs/tools.md)** for the tool reference, the wake gate, drawers, and token costs. Keep whichever one owns the surface updated when adding or changing user-facing behavior.
...
- [ ] **Tool schemas changed?** `npm run count-tokens` and update the token cost table in `docs/tools.md`.
- [ ] **User-facing behavior changed?** Update whichever of `README.md`, `docs/memory.md`, or `docs/tools.md` owns that surface.
...
7. **Wake gate** (`src/gate.ts`, `src/tools/wake.ts`) — added in 0.5.0. Every tool registers as normal, then `ToolGate.arm()` flips `enabled = false` on each `RegisteredTool` handle, so a client's first `tools/list` returns only `wake_betty`. Calling it opens the gate and sends one `tools/list_changed` for the whole batch (setting `enabled` directly rather than calling the SDK's per-tool `enable()`, which would emit one notification per tool).
   ...
   **It re-arms.** MCP has no per-conversation signal, so the gate is per-*connection*: on a host that keeps one process across chats, waking would otherwise be permanent. `BETTY_WAKE_REARM_MINUTES` (default 10) closes it after idle time; the sweep interval lives in `connectAll`, never `registerAll`, which stays free of I/O *and timers*. Re-arming mid-conversation is affordable only because of the `loaded` parameter: a model that already has the instructions gets the tools back for a sentence. Don't remove it. The SDK's rejection message for a disabled tool is a fixed `Tool X disabled` that can't be customized, so recovery depends entirely on the client acting on `list_changed`.
   ...
   `wake_betty`'s definition sits in every request's context forever, so it is a token budget as much as a tool: ~104 tokens against ~2,062 fully open (ungated; the awake tier is ~2,166, since `wake_betty` stays visible). `wake.test.ts` asserts the ceiling. Resist adding parameters or prose — the *reply* is where to spend words, since it is paid once per wake.
...
- **User-disablable tools**: `DISABLED_TOOLS` env var prevents specific tools from being registered. See `docs/tools.md` for details.
...
- **Betty is a colleague, not a costume**: the host model *works with* Betty; it never becomes her. `wake-betty` opens by saying so outright, because a skill whose first line is `# Betty` otherwise reads as a character sheet and invites a model to answer in her voice or narrate its own filing. Don't strip that guard — `bundled.test.ts` pins it.
  ...
  Two rules govern how she is described, in the README, in the skills, and in comments here:

  1. **Describe her by what she can do** — never her appearance, her manner, or her tenure. The persona (keeps the records, never forgets, never overwrites, doesn't announce the filing) is a mnemonic for behaviour the code already enforces, not decoration; keep the two in step. This is also why the prose is short: there is nothing to say about her beyond her capabilities, and a character sketch invites a model to perform one. The version before 0.7.2 described reading glasses on a gold chain, twenty years, and never raising her voice, which said nothing true about the software and leant on the mid-century secretary archetype to fill the gap.
  2. **She works for the user; she is not the user's.** Possessive framing ("she's yours", "your Betty") reads as owning a person and is worse than the loyalty language it replaced. The user owns the storage and the records — Betty is who keeps them. What made the old "loyal to *you*, not to the building" worth keeping is a fact about the software, so state it as one: her memory is markdown in storage the user controls, so she answers to them rather than to whichever platform is hosting the conversation.
...
3. `npm run count-tokens` and update the token cost table in `docs/tools.md` if tools changed.
...
`docs/tools.md` has a "Tool definition token cost" table in the Token efficiency section, plus a tier table below it. Keep both updated when tools are added, removed, or renamed — note the tier rows include `wake_betty`, which stays visible after waking, while the per-protocol rows above do not. Run `npm run count-tokens` to regenerate the numbers — it builds the project, loads the actual tool registrations with mock backends, and prints per-tool and per-configuration estimates (~3.5 chars/token heuristic for BPE on JSON Schema). The script lives at `scripts/count-tool-tokens.js`.
